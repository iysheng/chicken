...
SD_Init (void)
{
  ...
  SDCARD_Handler.Init.BusWide = SDMMC_BUS_WIDE_4B;                      //1位数据线
  ...
  if (SD_Error != HAL_OK)
    return 1;
  //if (SDMMC_BUS_WIDE_4B==SDCARD_Handler.Init.BusWide)
  //SD_Error=SD_WideBus_Enable(&SDCARD_Handler);//使能宽总线模式
...
//返回值:错误状态
uint8_t
SD_GetCardInfo (HAL_SD_CardInfoTypeDef *cardinfo)
{
...
//返回值:错误状态;0,正常;其他,错误代码;
uint8_t
SD_ReadDisk (uint8_t* buf, uint32_t sector, uint8_t cnt)
{
  uint8_t sta = HAL_OK;
  long long lsector = sector;
  uint8_t n;
  lsector <<= 9;
  OSIntEnter (); //关闭总中断(POLLING模式,严禁中断打断SDIO读写操作!!!)
  if ((uint32_t) buf % 4 != 0)
    {
      for (n = 0; n < cnt; n++)
	{
	  sta = HAL_SD_ReadBlocks (&SDCARD_Handler, (uint8_t*) SDIO_DATA_BUFFER,
				   lsector + 512 * n, 512, 1); //单个sector的读操作
	  memcpy (buf, SDIO_DATA_BUFFER, 512);
	  buf += 512;
	}
    }
    ...
    {
      sta = HAL_SD_ReadBlocks (&SDCARD_Handler, (uint8_t*) buf, lsector, 512,
			       cnt); //单个sector的读操作
    }
  OSIntExit (); //开启总中断
  return sta;
...
//返回值:错误状态;0,正常;其他,错误代码;
uint8_t
SD_WriteDisk (uint8_t *buf, uint32_t sector, uint8_t cnt)
{
  uint8_t sta = HAL_OK;
  long long lsector = sector;
  uint8_t n;
  ...
  if ((uint32_t) buf % 4 != 0)
	...
	{
	  memcpy (SDIO_DATA_BUFFER, buf, 512);
	  sta = HAL_SD_WriteBlocks (&SDCARD_Handler,
				    (uint8_t*) SDIO_DATA_BUFFER,
				    lsector + 512 * n, 512, 1); //单个sector的写操作
	  buf += 512;
	}
    }
  else
    {
      sta = HAL_SD_WriteBlocks (&SDCARD_Handler, (uint8_t*) buf, lsector, 512,
				cnt); //多个sector的写操作
    }
  OSIntExit (); //开启总中断
  return sta;
