    ...
    if(MSC_Handle->current_lun < MSC_Handle->max_lun)
    {
      ...
      case MSC_READ_CAPACITY10:   
        ...
        if(scsi_status == USBH_OK)
        {
          ...
          {
            USBH_UsrLog ("MSC Device capacity : %lu MBytes", \
              (int32_t)(((MSC_Handle->unit[MSC_Handle->current_lun].capacity.block_nbr * MSC_Handle->unit[MSC_Handle->current_lun].capacity.block_size))>>20));
