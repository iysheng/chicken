...
uint8_t Rx_Buff[ETH_RXBUFNB][ETH_RX_BUF_SIZE]; 		//以太网底层驱动接收buffers指针
uint8_t Tx_Buff[ETH_TXBUFNB][ETH_TX_BUF_SIZE]; 		//以太网底层驱动发送buffers指针
...
LAN8720_Init (void)
{
  ...
  uint32_t ret;
  //OSIntEnter();                         //关闭所有中断，复位过程不能被打断！
  __HAL_RCC_GPIOA_CLK_ENABLE()
  ;
  ETH_RST(0);       //硬件复位
  PCF8574_WriteBit(ETH_RESET_IO,1);       //硬件复位
  delay_ms(100);
  PCF8574_WriteBit(ETH_RESET_IO,0);       //硬件复位
  ETH_RST(1);       //复位结束
  delay_ms(100);
  //OSIntExit();                          //开启所有中断
  ...
  ret = HAL_ETH_Init (&ETH_Handler); //debug过程中修改了，HAL库代码stm32f7xx_hal_eth.c Line215
  if (ret == HAL_OK)
    return 0;   //成功
  else
    return 1;  //失败
...
extern void
lwip_pkt_handle (void);		//在lwip_comm.c里面定义
